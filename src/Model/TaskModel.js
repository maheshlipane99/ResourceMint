...
export default class TaskModel {
    ...
    static getCount = async () => {
        ...
        let r = realm.objects(tableName).filtered(`typeOfComplaintFK == 1`);
    ...
    static getPendingCount = async () => {
        let result = { data: [], message: '' }
        let r = realm.objects(tableName).filtered(`isSubmitted == false AND isDraft == false AND typeOfComplaintFK == 1`);
        result.data = r.length;
        if (r.length == 0) {
            result.message = "No record found"
        }
    ...
    static getAllItemsByLimit = async (currentCount) => {
        ...
        try {
            let data = realm.objects(tableName).sorted('complaintId', true).filtered(`complaintId != 0 AND serverId != 0 AND typeOfComplaintFK == 1 AND isSubmitted==true AND isMy==false`).slice(mFrom, mTo)
            if (data != null) {
                result.data = data
    ...
    static getMyTask = async (currentCount) => {
        let mFrom = currentCount
        let mTo = (currentCount + 20)
        let result = { data: null, message: '', mCount: 0 }
        try {
            let data = realm.objects(tableName).sorted('complaintId', true).filtered(`typeOfComplaintFK == 1 AND isMy==true`).slice(mFrom, mTo)
            if (data != null) {
                result.data = data
                result.mCount = result.length
            } else {
    ...
    static getDraftItem = async () => {
        ...
        try {
            realm.write(() => {
                let result = realm.objects(tableName).filtered(`isDraft == true AND typeOfComplaintFK == 1`);;
                if (result != null && result[0] != null) {
                    emp = result[0];
                } else {
                    message = "No record found"
