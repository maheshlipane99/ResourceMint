...
const tableName=Tables.LOCATION_TYPE;

export default class LocationTypeModel {
   ...
   static getCount= async () => {
    ...
    static getAllItemsByLimit = async (currentCount) => {
        ...
        try {
            let data = realm.objects(tableName).sorted('title', false).filtered(`isDeleted == 0`).slice(mFrom, mTo)
            if (data != null) {
                result.data = data
    ...
    static getItemById = async (id) => {
        ...
        try {
            realm.write(() => {
                let result = realm.objects(tableName).filtered(`locationTypeId == "${id}"`);;
