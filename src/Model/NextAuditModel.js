...
const tableName = Tables.NEXT_AUDIT;

export default class NextAuditModel {
    ...
    static getItemByAssetCatId = async (assetId,checkListId) => {
        let result = { data: null, message: '' }
        let emp = null
        let message = ''
        try {
            realm.write(() => {
                let result = realm.objects(tableName).filtered(`assetIdFK == "${assetId}" AND checkListIdFK == "${checkListId}"`);;
                if (result != null && result[0] != null) {
                    emp = result[0];
                } else {
                    message = "No record found"
    ...
    static getAllPendingItems = async () => {
        let result = { data: [], message: '' }
        let r = realm.objects(tableName).filtered(`isSubmitted == false `);
        result.data = r;
        if (result.data == null) {
            result.message = "No record found"
        }
