...
const tableName = Tables.CHEKLIST_IMAGE;

export default class ChekImageModel {
    ...
    static getAllItemsByLimit = async (currentCount) => {
        ...
        try {
            let data = realm.objects(tableName).sorted('imageId', true).slice(mFrom, mTo)
            if (data != null) {
                result.data = data
    ...
    static getItemById = async (id) => {
        ...
        try {
            realm.write(() => {
                let result = realm.objects(tableName).filtered(`imageId == "${id}"`);;
