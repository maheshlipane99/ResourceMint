import React, { Component } from 'react';
import { View, Text, TouchableOpacity, StyleSheet, Image, } from 'react-native';
import { RadioButton, TouchableRipple } from 'react-native-paper';

export default class RowComTrack extends Component {
    constructor(props) {
        super(props);
        this.state = {
            defaultImage: "data:image/png;base64,",
        }
    }
    render() {
        return (
            <View
                style={styles.itemContainer}
                onPress={!this.props.item.isDisable ? () => this.props.onPress('rootViewTrack') : null}
                rippleColor="rgba(0, 0, 0, .32)">
                <View style={{ backgroundColor: 'white', borderRadius: 10, borderColor:'#E9E6E5', borderWidth: 1 }}>
                    <View style={{ flexDirection: 'row', padding: 10 }}>
                        <Image style={[{ width: 38, height: 38, borderRadius: 38 / 2, borderWidth: 0.5, borderColor: '#646464' ,marginRight:10}]} source={{ uri: this.props.item.userProfile ? this.props.item.userProfile : this.state.defaultImage }} />
                        <View style={styles.container} >
                            <View>
                                <Text style={[styles.textName, { color: this.props.item.isChecked ? 'black' : '#707070' }]}>{this.props.item.userName}</Text>
                                <Text style={styles.textLabel}>Role :   <Text style={[styles.textContent]}>{this.props.item.typeOfUser}</Text></Text>
                                <Text style={styles.textLabel}>Action :  <Text style={[styles.textContent]}>{this.props.item.complaintStatus}</Text></Text>
                                <Text style={styles.textLabel}>Date :  <Text style={[styles.textContent]}>{this.props.item.createdDate}</Text></Text>
                            </View>
                        </View>
                    </View>
                </View>
            </View>
        );
    }
}


const styles = StyleSheet.create({
    itemContainer: {
        margin: 10,
    },
    container: {
        flex: 1,
        justifyContent: 'center',
        alignContent: 'center',
    },
    textName: {
        justifyContent: 'center',
        fontSize: 14,
        color:'#2D2D2D',
        fontFamily: 'Barlow-Medium',
        alignSelf: 'flex-start',
        alignContent: 'center',
        textAlign: 'center',
    },
    textLabel: {
        justifyContent: 'center',
        fontSize: 12,
        color:'#707070',
        fontFamily: 'Barlow-Light',
        alignSelf: 'flex-start',
        alignContent: 'center',
        textAlign: 'center',
    },
    textContent: {
        justifyContent: 'center',
        fontSize: 12,
        color:'#C1C0C0',
        fontFamily: 'Barlow-Light',
        alignSelf: 'flex-start',
        alignContent: 'center',
        textAlign: 'center',
    },
})