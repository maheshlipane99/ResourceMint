...
class Manufacturer extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            manufacturerId: props.navigation.state.params.manufacturerId,
            header: 'Select Manufacturer',
            mToken: '',
            statusId: '',
            title: '',
            btnActive: true,
    ...
    getData1 = () => {
        ManufacturerModel.getAllItemsByLimit(this.state.data.length).then(result => {
            ...
            if (result.data.length > 0) {
                ...
                for (let index = 0; index < mDataList.length; index++) {
                    ...
                    if (element.manufacturerId == this.state.manufacturerId) {
    ...
    downloadStatus = () => {
        BackgroundService.getManufacturer(this.state.mToken).then((result) => {
    ...
    changeStatus = (index1) => {
        this.setState({ isLoading: true }, () => {
            ...
            let newData = []
            let btnActive, manufacturerId,title
            ...
            this.setState({ data: newData, message: '', animating: false, btnActive, manufacturerId, isLoading: false ,title});
        });
    ...
    onClickHandler(id) {
        switch (id) {
            ...
            case 'change': {
                this.props.navigation.state.params.returnData({ manufacturerId: this.state.manufacturerId ,title: this.state.title});
