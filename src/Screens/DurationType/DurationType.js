...
class DurationType extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            durationTypeId: props.navigation.state.params.durationTypeId,
            header: 'Select Duration Type',
            mToken: '',
            statusId: '',
            title: '',
            btnActive: true,
    ...
    getData1 = () => {
        DurationTypeModel.getAllItemsByLimit(this.state.data.length).then(result => {
            ...
            if (result.data.length > 0) {
                ...
                for (let index = 0; index < mDataList.length; index++) {
                    ...
                    if (element.durationTypeId == this.state.durationTypeId) {
    ...
    downloadStatus = () => {
        BackgroundService.getDurationType(this.state.mToken).then((result) => {
    ...
    changeStatus = (index1) => {
        this.setState({ isLoading: true }, () => {
            ...
            let newData = []
            let btnActive, durationTypeId,title
                ...
                let mItem = this.state.data[index]
                    if (index == index1) {
                        ...
                        if (!mItem.isChecked) {
                            ...
                            btnActive = false
                            durationTypeId = mItem.durationTypeId
                            title=mItem.title
                        }
                    } else {
            ...
            this.setState({ data: newData, message: '', animating: false, btnActive, durationTypeId, isLoading: false ,title});
        });
    ...
    onClickHandler(id) {
        switch (id) {
            ...
            case 'change': {
                this.props.navigation.state.params.returnData({ durationTypeId: this.state.durationTypeId ,title: this.state.title});
