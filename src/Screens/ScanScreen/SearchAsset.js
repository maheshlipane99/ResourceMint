...
import RowRecentAsset from '../Row/RowRecentAsset'
...
class SearchAsset extends Component {
    ...
    getData = () => {
        RecentAssetModel.getAllItems().then(result => {
            console.log(JSON.stringify(result.data));
            if (result.data != null) {
                this.setState({ data: result.data, message: '', animating: false });
            } else {
                this.setState({ message: 'No Recent item', animating: false, title: '' });
            }
    ...
    onItemClickHandler(id, index) {
        switch (id) {
            case 'rootView': {
                let assetCode = this.state.data[index].assetCode
                this.checkAssetOffline(assetCode);
                break;
            }
        }
    }
    ...
    searchAsset = (assetCode) => {
        ...
        AssetModel.searchAsset(assetCode + "").then(result => {
    ...
    checkAssetOffline = (assetCode) => {
        this.setState({ snackVisible: false, message1: '' })
        AssetModel.getItemByAssetCode(assetCode + "").then(result => {
            let mData = result.data
            if (mData) {
                console.log(mData);
                this.goNext(mData);
    ...
    goNext = (data) => {
        if (data != null) {
            ...
            this.props.navigation.state.params.returnData({ assetId: data.assetId, assetCode: data.assetCode });
        }
    }
            ...
            masterView = (
                <View style={{ flex: 1, flexDirection: 'column' }}>
                    ...
                    <Card style={{ flex: 1, margin: 10, backgroundColor: 'white', radius: 10, padding: 15, borderColor: '#E9E6E5' }} elevation={5}>
                        ...
                        />
                        <Text style={styles.textLight}>Type code to get asset details.</Text>
                        <FlatList
                            keyboardShouldPersistTaps="always"
                            style={{ paddingLeft: 5, paddingRight: 5 }}
                            data={this.state.data}
                            renderItem={({ item, index }) => {
                                return <RowRecentAsset item={item} index={index} onPress={(action) => this.onItemClickHandler(action, index)} />;
                            }}
                    ...
                    {/* <Button
                        ...
                        onPress={() => { this.onClickHandler('search') }}  >{this.state.isFetching ? 'Wait...' : 'Get Details'}</Button> */}
