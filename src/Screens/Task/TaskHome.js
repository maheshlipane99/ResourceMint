...
class TaskHome extends Component {
    ...
    initFeature = () => {
        FeaturesModel.getItemById(FEATURES.Task_Creation).then(result => {
    ...
    onClickHandler(id) {
        switch (id) {
            ...
            case 'add': {
                Navigation.navigate('CreateTask', { returnData: this.returnData.bind(this) });
                break;
            }
        }
    }
    ...
    returnData = (data) => {
        ...
        if (data.status) {
            this.props.dataChanged({ isUpdated: true, type: 'createTask' });
            this.props.dataChanged({ isUpdated: false, type: 'createTask' });
                ...
                <ImageBackground source={require('../../Assets/Header/header_bg.png')} style={styles.backImage}  >
                    <View style={{ flex: 1, width: '100%', height: '100%', flexDirection: 'row' }}>
                        <TouchableOpacity onPress={() => this.onClickHandler('1')} style={styles.context}>
                            <Text style={(this.state.position == 0) ? styles.activeText : styles.inActiveText}  >Tasks</Text>
                        ...
                        <TouchableOpacity onPress={() => this.onClickHandler('2')} style={styles.context}>
                            <Text style={(this.state.position == 0) ? styles.inActiveText : styles.activeText} >My Tasks</Text>
...
export default connect(mapStateToProps, { dataChanged })(TaskHome);
