...
class TaskList extends Component {
    ...
    getData = () => {
        TaskModel.getAllItemsByLimit(this.state.data.length).then(result => {
    ...
    getServerData = () => {
        BackgroundService.getTasks(this.state.mToken, 1).then((result) => {
            if (result) {
                this.setState({ data: [] }, () => {
                    this.getData()
                });
        ...
        BackgroundService.getTransTasks(this.state.mToken, 1).then((result) => {
            if (result) {
                this.setState({ data: [] }, () => {
                    this.getData()
                });
...
export default connect(mapStateToProps, { dataChanged })(TaskList);
