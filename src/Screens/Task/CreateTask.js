...
const messageSuccess = 'This task saved offline, until network availability.';
...
class CreateTask extends Component {
    ...
    checkDraft() {
        TaskModel.getDraftItem().then(result => {
    ...
    saveDraft = (callback) => {
        this.deletePhoto(this.state.complaintImages[this.state.complaintImages.length - 1], () => {
            ...
            if (this.state.isDraft) {
                ...
                TaskModel.updateItem(complaint).then((result) => {
                ...
                TaskModel.addItem(complaint).then((result) => {
                    console.log(JSON.stringify(result));
                    this.setState({ complaintImages: [] }, () => {
                        callback()
                    })
    ...
    raiseComplaint = () => {
        this.deletePhoto(this.state.complaintImages[this.state.complaintImages.length - 1], () => {
            ...
            if (this.state.isDraft) {
                ...
                TaskModel.updateItem(complaint).then((result) => {
                    console.log(JSON.stringify(result));
                    BackgroundService.startTaskUploading(this.state.mToken).then(() => {
                        // this.props.dataChanged({ isUpdated: true, type: 'task' });
                        // this.props.dataChanged({ isUpdated: false, type: 'task' });
                    })
            ...
            submitedView = (
                <View style={{ height: '100%', alignContent: 'center', justifyContent: 'center', padding: 20 }}>
                    <Done
                        ...
                        icon='check-circle'
                        onPress={() => { this.onClickHandler('myTask') }} />
                </View>
            )
        }
                    ...
                    <View>
                        ...
                        <View style={{ padding: 10 }}>
                            <Button
                                ...
                                onPress={() => { this.onClickHandler('raise') }}  >{this.state.selectedUser.length == 0 ? 'Select User' : 'Create Task'}</Button>
                        </View>
                    </View>
                </ ScrollView>
            )
...
export default connect(mapStateToProps, { dataChanged })(CreateTask);
