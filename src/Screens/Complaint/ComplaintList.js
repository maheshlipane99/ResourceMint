...
class ComplaintList extends Component {
    ...
    getServerData = () => {
        BackgroundService.getComplaints(this.state.mToken, 1).then((result) => {
            if (result) {
                this.setState({ data: [] }, () => {
                    this.getData()
                });
        ...
        BackgroundService.getTransComplaints(this.state.mToken, 1).then((result) => {
            if (result) {
                this.setState({ data: [] }, () => {
                    this.getData()
                });
...
export default connect(mapStateToProps, { dataChanged })(ComplaintList);
