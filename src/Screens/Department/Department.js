...
class Department extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            departmentId: props.navigation.state.params.departmentId,
    ...
    getData1 = () => {
        DepartmentModel.getAllItemsByLimit(this.state.data.length).then(result => {
            ...
            if (result.data.length > 0) {
                ...
                for (let index = 0; index < mDataList.length; index++) {
                    ...
                    if (element.departmentId == this.state.departmentId) {
    ...
    downloadStatus = () => {
        BackgroundService.getDepartment(this.state.mToken).then((result) => {
    ...
    changeStatus = (index1) => {
        this.setState({ isLoading: true }, () => {
            ...
            let newData = []
            let btnActive, departmentId, title
            for (let index = 0; index < mData.length; index++) {
                ...
                if (index == index1) {
                    ...
                    if (!mItem.isChecked) {
                        ...
                        btnActive = false
                        departmentId = mItem.departmentId
                        title = mItem.title
                    }
                } else {
            ...
            this.setState({ data: newData, message: '', animating: false, btnActive, departmentId, isLoading: false, title });
        });
    ...
    onClickHandler(id) {
        switch (id) {
            ...
            case 'change': {
                this.props.navigation.state.params.returnData({ departmentId: this.state.departmentId, title: this.state.title });
