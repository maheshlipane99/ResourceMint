...
class Supplier extends Component {
    ...
    getData1 = () => {
        SupplierModel.getAllItemsByLimit(this.state.data.length).then(result => {
            ...
            if (result.data.length > 0) {
                ...
                for (let index = 0; index < mDataList.length; index++) {
                    ...
                    if (element.supplierId == this.state.supplierId) {
    ...
    downloadStatus = () => {
        BackgroundService.getSupplier(this.state.mToken).then((result) => {
    ...
    changeStatus = (index1) => {
        this.setState({ isLoading: true }, () => {
            ...
            let newData = []
            let btnActive, supplierId,title
            ...
            this.setState({ data: newData, message: '', animating: false, btnActive, supplierId, isLoading: false ,title});
        });
    ...
    onClickHandler(id) {
        switch (id) {
            ...
            case 'change': {
                this.props.navigation.state.params.returnData({ supplierId: this.state.supplierId ,title: this.state.title});
